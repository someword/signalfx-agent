...
def test_elasticsearch_in_k8s(agent_image, minikube, k8s_observer, k8s_test_timeout):
    monitors = [
        {"type": "collectd/elasticsearch",
         "discoveryRule": 'container_image =~ "elasticsearch" && private_port == 9200',
         "username": "testuser", "password": "testing123"},
    ]
    yamls = [os.path.join(os.path.dirname(os.path.realpath(__file__)), y) for y in ["elasticsearch.yaml"]]
    run_k8s_monitors_test(
        agent_image,
        minikube,
        monitors,
