...
def test_zookeeper_in_k8s(agent_image, minikube, k8s_observer, k8s_test_timeout):
    monitors = [
        {"type": "collectd/zookeeper",
         "discoveryRule": 'container_image =~ "zookeeper" && private_port == 2181'}
    ]
    yamls = [os.path.join(os.path.dirname(os.path.realpath(__file__)), y) for y in ["zookeeper.yaml"]]
    run_k8s_monitors_test(
        agent_image,
        minikube,
        monitors,
