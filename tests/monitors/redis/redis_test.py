...
def test_redis_in_k8s(agent_image, minikube, k8s_observer, k8s_test_timeout):
    monitors = [
        {"type": "collectd/redis",
         "discoveryRule": 'container_image =~ "redis" && private_port == 6379'}
    ]
    yamls = [os.path.join(os.path.dirname(os.path.realpath(__file__)), y) for y in ["redis.yaml"]]
    run_k8s_monitors_test(
        agent_image,
        minikube,
        monitors,
